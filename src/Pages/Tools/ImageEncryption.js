...
// reuse same binary envelope and crypto helpers as file tools
...
export default function ImageEncryption(){
  ...
  async function handleEncrypt(){
    setError('');
    if(!file) return setError('Choose an image to encrypt');
    ...
    try{
      ...
      sessionStorage.setItem('lastAction', 'encrypted-image');
      navigate('/tools/file-secure/image-decryption');
      ...
      <div className="card-wrap">
        <div className="content private">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ flex: 1 }}>
              <div className="title"><span className="first-word">Image </span>Encryption</div>
              <div style={{ marginTop: 10, color: '#555' }}>Select an image and a passphrase.</div>
            </div>
            <div>
              <button type="button" onClick={(e)=>{ e.preventDefault(); navigate('/tools/file-secure/image-decryption'); }} style={{ padding: '8px 12px', background: '#0b76ef', color: '#fff', borderRadius: 8, border: 'none', fontWeight: 600 }}>Decrypt Image</button>
            </div>
          </div>
          ...
          <div style={{ marginTop: 14 }}>
            ...
            <div style={{ marginTop: 8 }}>
              ...
              {preview && <div style={{ marginTop: 12 }}><img src={preview} alt="preview" style={{ maxWidth: '100%', borderRadius: 8, boxShadow: '0 6px 18px rgba(0,0,0,0.08)' }} /></div>}
            </div>
          </div>

...
export default function AudioDecryption(){
  ...
  useEffect(()=>{
    ...
    if(lastAction === 'encrypted-audio'){
      setSuccess('Audio encrypted and downloaded — upload it here to decrypt.');
  ...
  async function handleDecrypt(){
    ...
    try{
      ...
      setSuccess('Audio decrypted and downloaded successfully.');
      ...
      <div className="card-wrap">
        <div className="content private">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ flex: 1 }}>
              <div className="title"><span className="first-word">Audio </span>Decryption</div>
              <div style={{ marginTop: 10, color: '#555' }}>Upload the encrypted audio and enter a passphrase.</div>
            </div>
            <div>
              <button type="button" onClick={(e)=>{ e.preventDefault(); window.location.href = '/tools/file-secure/audio-encryption'; }} style={{ padding: '8px 12px', background: '#0b76ef', color: '#fff', borderRadius: 8, border: 'none', fontWeight: 600 }}>Encrypt Audio</button>
            </div>
          </div>

          <div style={{ marginTop: 14 }}>
            <label>Encrypted Audio</label>

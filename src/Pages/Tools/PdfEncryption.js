...
export default function PdfEncryption(){
  ...
  function handleFileChange(e){
    ...
    setFile(f);
    setError('');
    if(f){ const url = URL.createObjectURL(f); setPreview(url); } else { if(preview){ URL.revokeObjectURL(preview); setPreview(''); } }
  }

  async function handleEncrypt(){
    setError('');
    if(!file) return setError('Choose a PDF to encrypt');
    ...
    try{
      ...
      a.href = url; a.download = `${file.name}.enc`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
      sessionStorage.setItem('lastFilePass', pass);
      sessionStorage.setItem('lastAction', 'encrypted-pdf');
      navigate('/tools/file-secure/pdf-decryption');
      ...
      <div className="card-wrap">
        <div className="content private">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ flex: 1 }}>
              <div className="title"><span className="first-word">PDF </span>Encryption</div>
              <div style={{ marginTop: 10, color: '#555' }}>Select a PDF and a passphrase.</div>
            </div>
            <div>
              <button type="button" onClick={(e)=>{ e.preventDefault(); navigate('/tools/file-secure/pdf-decryption'); }} style={{ padding: '8px 12px', background: '#0b76ef', color: '#fff', borderRadius: 8, border: 'none', fontWeight: 600 }}>Decrypt PDF</button>
            </div>
          </div>
          ...
          <div style={{ marginTop: 14 }}>
            ...
            <div style={{ marginTop: 8 }}>
              ...
              <div style={{ marginTop: 8, color: '#666' }}>{file ? file.name : 'No PDF selected'}</div>
              {preview && <div style={{ marginTop: 12 }}><embed src={preview} type="application/pdf" width="100%" height={400} /></div>}
            </div>
          </div>

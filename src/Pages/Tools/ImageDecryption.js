...
export default function ImageDecryption(){
  ...
  useEffect(()=>{
    ...
    if(lastAction === 'encrypted-image'){
      setSuccess('Image encrypted and downloaded — upload it here to decrypt.');
  ...
  async function handleDecrypt(){
    ...
    try{
      ...
      setSuccess('Image decrypted and downloaded successfully.');
      ...
      <div className="card-wrap">
        <div className="content private">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ flex: 1 }}>
              <div className="title"><span className="first-word">Image </span>Decryption</div>
              <div style={{ marginTop: 10, color: '#555' }}>Upload the encrypted image and enter a passphrase.</div>
            </div>
            <div>
              <button type="button" onClick={(e)=>{ e.preventDefault(); window.location.href = '/tools/file-secure/image-encryption'; }} style={{ padding: '8px 12px', background: '#0b76ef', color: '#fff', borderRadius: 8, border: 'none', fontWeight: 600 }}>Encrypt Image</button>
            </div>
          </div>

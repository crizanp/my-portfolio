...
export default function VideoDecryption(){
  ...
  useEffect(()=>{
    ...
    if(lastAction === 'encrypted-video'){
      setSuccess('Video encrypted and downloaded — upload it here to decrypt.');
  ...
  async function handleDecrypt(){
    ...
    try{
      ...
      setSuccess('Video decrypted and downloaded successfully.');
      ...
      <div className="card-wrap">
        <div className="content private">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ flex: 1 }}>
              <div className="title"><span className="first-word">Video </span>Decryption</div>
              <div style={{ marginTop: 10, color: '#555' }}>Upload the encrypted video and enter a passphrase.</div>
            </div>
            <div>
              <button type="button" onClick={(e)=>{ e.preventDefault(); window.location.href = '/tools/file-secure/video-encryption'; }} style={{ padding: '8px 12px', background: '#0b76ef', color: '#fff', borderRadius: 8, border: 'none', fontWeight: 600 }}>Encrypt Video</button>
            </div>
          </div>

          <div style={{ marginTop: 14 }}>
            <label>Encrypted Video</label>

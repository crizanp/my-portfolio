...
export default function AudioEncryption(){
  ...
  async function handleEncrypt(){
    setError('');
    if(!file) return setError('Choose an audio file to encrypt');
    ...
    try{
      ...
      sessionStorage.setItem('lastAction', 'encrypted-audio');
      navigate('/tools/file-secure/audio-decryption');
      ...
      <div className="card-wrap">
        <div className="content private">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ flex: 1 }}>
              <div className="title"><span className="first-word">Audio </span>Encryption</div>
              <div style={{ marginTop: 10, color: '#555' }}>Select an audio file and a passphrase.</div>
            </div>
            <div>
              <button type="button" onClick={(e)=>{ e.preventDefault(); navigate('/tools/file-secure/audio-decryption'); }} style={{ padding: '8px 12px', background: '#0b76ef', color: '#fff', borderRadius: 8, border: 'none', fontWeight: 600 }}>Decrypt Audio</button>
            </div>
          </div>
          ...
          <div style={{ marginTop: 14 }}>
            ...
            <div style={{ marginTop: 8 }}>
              ...
              <div style={{ marginTop: 8, color: '#666' }}>{file ? file.name : 'No audio selected'}</div>
              {preview && <div style={{ marginTop: 12 }}><audio controls src={preview} style={{ width: '100%' }} /></div>}
            </div>
          </div>
